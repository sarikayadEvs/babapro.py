...
def get_min_followers():
    while True:
        try:
            return int(input(f'{Fore.YELLOW}Minimum Followers: {Fore.WHITE}'))
        except ValueError:
...
def check_tiktok(email):
    ...
    if response and response.get('status') == 'registered':
        print(f'{Fore.GREEN}[+] TikTok Kayıtlı : {email}{Fore.RESET}')
        return True
    print(f'{Fore.RED}[-] TikTok Kayıtlı Değil: {email}{Fore.RESET}')
    return False
...
def check_gmail(email):
    ...
    if response and response.get('status') == 'available':
        print(f'{Fore.GREEN}[+] Gmail Alinamadi: {email}{Fore.RESET}')
        return True
    print(f'{Fore.RED}[-] Gmail Alındı: {email}{Fore.RESET}')
    return False
...
def capture(email):
    ...
    try:
        ...
        # Save to file
        with open('Kawixxhits.txt', 'a', encoding='utf-8') as f:
            f.write(f"\n{'='*50}\n")
...
if __name__ == "__main__":
    print(f"{Fore.CYAN}TikTok Scraper Başlatıldı (Min. Takipçi: {min_followers}){Fore.RESET}")
    ...
    try:
        ...
        print(f"\n{Fore.YELLOW}Scraper Durduruldu...{Fore.RESET}")
